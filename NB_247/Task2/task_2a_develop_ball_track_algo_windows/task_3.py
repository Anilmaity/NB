...
def init_setup(rec_client_id):
    ...
    """
    global client_id, vision_sensor_handle, servoHandler, servoPosition
    ...
    code, vision_sensor_handle = sim.simxGetObjectHandle(client_id, 'vision_sensor_1', sim.simx_opmode_blocking)

    for i in range(0, 8):
        code, servo = sim.simxGetObjectHandle(client_id, 'revolute_joint_ss_' + str(i + 1), sim.simx_opmode_blocking)
        returnCode = sim.simxSetJointTargetPosition(client_id, servo, 0, sim.simx_opmode_oneshot_wait)
        servoHandle.append(servo)

    servoHandler = servoHandle

    servo = [0, 0, 0, 0, 0, 0, 0, 0]

    servoPosition = [0, 0, 0, 0, 0, 0, 0, 0]

    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[0], servo[0], sim.simx_opmode_oneshot)
    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[1], servo[1], sim.simx_opmode_oneshot)
    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[2], servo[2], sim.simx_opmode_oneshot)
    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[3], servo[3], sim.simx_opmode_oneshot)
    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[4], servo[4], sim.simx_opmode_oneshot)
    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[5], servo[5], sim.simx_opmode_oneshot)
    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[6], servo[6], sim.simx_opmode_oneshot)
    returnCode = sim.simxSetJointTargetPosition(client_id, servoHandler[7], servo[7], sim.simx_opmode_oneshot)
...
R_I = 0
last_angle_value_roll = 0


def calculate_angle_value_roll(roll_angle_error):


    global R_I, last_angle_value_roll , P,I,D

    P_value = P * (roll_angle_error)
    R_I += (I * roll_angle_error)
    D_value = D * (-roll_angle_error + last_angle_value_roll)
    roll_angle_value = P_value #+ R_I + D_value
    last_angle_value_roll = roll_angle_error

    if abs(roll_angle_value) >= 1.57/2:
        roll_angle_value = 1.57/2 * (roll_angle_value / abs(roll_angle_error))

    return roll_angle_value



P_I = 0
last_angle_value_pitch = 0


def calculate_angle_value_pitch(pitch_angle_error):
    global P_I, last_angle_value_pitch, P,I,D



    P_value = P * (pitch_angle_error)
    P_I += (I * pitch_angle_error)
    D_value = D * (-pitch_angle_error + last_angle_value_pitch)

    pitch_angle_value = P_value# + P_I + D_value
    last_angle_value_pitch = pitch_angle_error

    if abs(pitch_angle_value) >= 1.57/2:
        pitch_angle_value = 1.57/2 * (pitch_angle_value / abs(pitch_angle_value))

    return pitch_angle_value


def output(roll_value, pitch_value):
    servopos = []
    ...
    if abs((pitch_value) - (roll_value)) >= (1.57/2):
        servo[0] = (pitch_value) - (roll_value)/abs((pitch_value) - (roll_value)) * 1.57/2
    else:
        servo[0] = (pitch_value) - (roll_value)

    servo[1] = pitch_value

    if abs((pitch_value) + (roll_value)) >= (1.57/2):
        servo[2] = (pitch_value) + (roll_value)/abs((pitch_value) + (roll_value)) * 1.57/2
    else:
        servo[2] = (pitch_value) + (roll_value)

    if abs((-pitch_value) - (roll_value)) >= (1.57/2):
        servo[3] = (-pitch_value) - (roll_value)/abs((-pitch_value) - (roll_value)) * 1.57/2
    else:
        servo[3] = (-pitch_value) - (roll_value)

    servo[4] = (-pitch_value)


    if abs((-pitch_value) + (roll_value)) >= (1.57/2):
        servo[5] = (-pitch_value) + (roll_value)/abs((-pitch_value) + (roll_value)) * 1.57/2
    else:
        servo[5] = (-pitch_value) + (roll_value)
    ...
    #print('input for correction', roll_value, pitch_value, servoPosition, servo)
...
def position():
    ...
    z = []
    for d in range(0, 8):
        code, a = sim.simxGetJointPosition(client_id, servoHandler[d], sim.simx_opmode_oneshot)
        z.append((a))
    servoPosition = z


# print('actual servoposition' , z)
...
def control_logic(center_x, center_y):
    ...
    """
    global setpoint, client_id, start
    ...
    while (time.time() - startloop <= 0):
        ...
        # print(center_x, center_y , 'running control loop per frame ')

        roll_angle_error = ( 1.57*(center_y - 640) / 640)
        pitch_angle_error = (1.57* (center_x - 640) / 640)
        # roll_angle_error =0
        # pitch_angle_error = 0

        if abs(pitch_angle_error) > 0:
            pitch_value = calculate_angle_value_roll(pitch_angle_error)
        else:
            pitch_value = 0

        if abs(roll_angle_error) > 0:
            roll_value = calculate_angle_value_roll(roll_angle_error)
        else:
            roll_value = 0

        # print(roll_value,pitch_value)
        output(roll_value, pitch_value)
    #print('FPS = ', io)
                            ...
                            if (type(warped_img) is np.ndarray):
                                ...
                                try:
                                    ...
                                    if (type(shapes) is dict and shapes != {}):
                                        ...
                                        center_x = shapes['Circle'][0][1]
                                        center_y = shapes['Circle'][0][2]
